...
variable "mgmt_ip_values" {
  type        = map(string)
  default     = {}
  description = "Map of management appliance names to explicit IP addresses for custom modes."
  validation {
    condition = alltrue([
      for k, v in var.mgmt_ip_values : v == "" || can(regex("^(?:[0-9]{1,3}\\.){3}[0-9]{1,3}$", v)) || can(regex("^(?:[0-9]{1,3}\\.){3}[0-9]{1,3}-(?:[0-9]{1,3}\\.){3}[0-9]{1,3}$", v))
    ])
    error_message = "All non-empty IP values in mgmt_ip_values must be valid IPv4 address strings or IP range strings."
  }
}
...
variable "nsx_ip_values" {
  type        = map(string)
  default     = {}
  description = "Map of NSX datapath appliance names to explicit IP addresses for custom modes."
  validation {
    condition = alltrue([
      for k, v in var.nsx_ip_values : v == "" || can(regex("^(?:[0-9]{1,3}\\.){3}[0-9]{1,3}$", v)) || can(regex("^(?:[0-9]{1,3}\\.){3}[0-9]{1,3}-(?:[0-9]{1,3}\\.){3}[0-9]{1,3}$", v))
    ])
    error_message = "All non-empty IP values in nsx_ip_values must be valid IPv4 address strings or IP range strings."
  }
}
